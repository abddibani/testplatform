...
export class OrderService {
  ...
  update(order: IOrder): Observable<EntityResponseType> {
    const copy = this.convertDateFromClient(order);
    return this.http
      .put<IOrder>(`${this.resourceUrl}/${getOrderIdentifier(order) as string}`, copy, { observe: 'response' })
      .pipe(map((res: EntityResponseType) => this.convertDateFromServer(res)));
  }
  ...
  partialUpdate(order: IOrder): Observable<EntityResponseType> {
    const copy = this.convertDateFromClient(order);
    return this.http
      .patch<IOrder>(`${this.resourceUrl}/${getOrderIdentifier(order) as string}`, copy, { observe: 'response' })
      .pipe(map((res: EntityResponseType) => this.convertDateFromServer(res)));
  }
  ...
  protected convertDateFromClient(order: IOrder): IOrder {
    return Object.assign({}, order, {
      createAt: order.createAt?.isValid() ? order.createAt.format(DATE_FORMAT) : undefined,
      modifiedAt: order.modifiedAt?.isValid() ? order.modifiedAt.format(DATE_FORMAT) : undefined,
    });
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((order: IOrder) => {
        order.createAt = order.createAt ? dayjs(order.createAt) : undefined;
        order.modifiedAt = order.modifiedAt ? dayjs(order.modifiedAt) : undefined;
      });
    }
    return res;
  }
